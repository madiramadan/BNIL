...
class Merchandise_cat extends CI_Controller {
	...
	function merchandise_cat_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_merchandise_cat WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
...
unset($post['id']);
		...
		if(!$param['id']) {
			$post['id'] = GetValue("id","merchandise_cat",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		
		// upload file
		$config ['upload_path']   = '../uploads/merchandise_cat';
		...
		$cek_id = GetValue("id","merchandise_cat",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
...
$post['modify_date'] = date("Y-m-d H:i:s");
			...
			$record = GetAll('merchandise_cat',array('id'=>'where/'.$cek_id))->row_array();
			if (!$this->upload->do_upload('image')){
