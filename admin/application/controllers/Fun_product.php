...
class Fun_product extends CI_Controller {
	...
	function fun_product_list()
	{
		...
		$query 	= "SELECT * FROM kg_fun_product WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
  	...
  	function fun_product_detail($id=0)
	{
		...
		$data['functionz'] 	= 'fun_product';
		$data['id'] 		= $id;
		$data['category'] 	= GetAll('fun_product_cat', array('is_delete'=>'where/0','id_parent'=>'where/0'))->result_array();
		if($id) {
			$q = GetAll("fun_product", array("id" => "where/".$id,'is_delete'=>'where/0'))->result_array()[0];
...
unset($post['id']);
		...
		if(!$param['id']) {
			$post['id'] = GetValue("id","fun_product",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
			
		$cek_id = GetValue("id","fun_product",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
