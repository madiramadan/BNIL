...
class Financial_report extends CI_Controller {
	...
	function financial_report_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_financial_report WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
	...
	function financial_report_add()
	{
		...
		if(!$param['id']) {
			$post['id'] = GetValue("id","financial_report",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}

		// upload file
		$config['upload_path']          = '../uploads/financial_report';
		...
		$cek_id = GetValue("id","financial_report",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
