...
class Banner extends CI_Controller {
	...
	function banner_list($trash=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query = "SELECT * FROM kg_banner WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";
...
unset($post['id']);
    ...
    $post['slug'] = url_title($post['title']);
		...
		if(!isset($post['is_delete'])) $post['is_delete']=0;
		if(!$param['id']) {
			$post['id'] = GetValue("id","banner",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		//File
...
$post['modify_date'] = date("Y-m-d H:i:s");
  ...
  function banner_info()
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'banner_info';
    $data['filez'] = 'banner';
    $data['functionz'] = 'banner_info';
    $data['labelz'] = 'Info in Banner';
    $this->load->view('template', $data);
	}
  
  function banner_info_list($trash=0)
  {
  	ini_set('memory_limit', -1);
  	$where="1=1";
  	$kolom = array("","title","publish_date","is_publish","urutan");
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query_no_limit = "SELECT * FROM kg_banner_info WHERE is_delete=0 AND ".$where." ORDER BY ".$ord;
		$query = "SELECT * FROM kg_banner_info WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";
  	$list = $this->db->query($query);
  	$data = array();
	  $no = $_POST['start'];
	  foreach ($list->result_array() as $r) {
	  	$no++;
	  	//$chk = "<input type='checkbox' value='1' name='chk[".$r['id']."]' class='chk'>";
		  $check = "<a href='".site_url('banner/banner_info_detail/'.$r['id'])."'><i class='icon icon-pencil'></i></a>";
		  $check .= "&nbsp;&nbsp;<a href=''><i class='akt icon icon-trash' rel='".$r['id']."' zz='2'></i></a>";
		  $publish = $r['is_publish'] ? "Published" : "Draft";
		  $data[] = array($no, $r['title'], $r['publish_date'], $publish, $r['urutan'], $check);
	  }
  ...
  function banner_info_detail($id=0)
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'banner_info_detail';
    $data['filez'] = 'banner';
    $data['functionz'] = 'banner_info';
    $data['labelz'] = 'Info in Banner';
    $data['id'] = $id;
    if($id) {
    	$q = GetAll("banner_info", array("id"=> "where/".$id))->result_array()[0];
    	$data['val'] = $q;
    } else $data['val'] = array();
    $this->load->view('template', $data);
	}
	
	function banner_info_aktif()
	{
		$idz = $this->input->post("idz");
		if($idz) {
			if($this->input->post("akt")==2) {
				$this->db->where("id", $idz);
				$this->db->update("banner_info", array("is_delete"=> 1));
			}
		}
	}
  
  function banner_info_add()
  {
  	ini_set('memory_limit', -1);
    $admin_id=permission();
    $post = $param = $this->input->post();
		unset($post['id']);
    $post['slug'] = url_title($post['title']);
    $post['contents']=$post['contents_eng']="";
		if(!isset($post['is_publish'])) $post['is_publish']=0;
		if(!isset($post['is_delete'])) $post['is_delete']=0;
		if(!$param['id']) {
			$post['id'] = GetValue("id","banner_info",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		//File
		$flz = array("file","file_mobile");
		foreach($flz as $key) {
		...
		$cek_id = GetValue("id","banner_info",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
    	$post['create_user_id'] = $admin_id;
			$post['create_date'] = date("Y-m-d H:i:s");
$post['modify_user_id'] = $admin_id;
$post['modify_date'] = date("Y-m-d H:i:s");
			if($this->db->insert("banner_info", $post)) $this->session->set_flashdata("message", "success-Add Success");
    	else $this->session->set_flashdata("message", "danger-Add Failed");
    } else {
    	$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			$this->db->where("id", $param['id']);
    	if($this->db->update("banner_info", $post)) $this->session->set_flashdata("message", "success-Edit Success");
    	else $this->session->set_flashdata("message", "danger-Edit Failed");
    }
    redirect(site_url('banner/banner_info'));
  }
}
