    ...
    function admin_list($trash=0)
	  {
	  	...
	  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
			...
			$query_no_limit = "SELECT * FROM kg_admin WHERE ".$where." ORDER BY ".$ord;
			$query = "SELECT * FROM kg_admin WHERE ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
		  ...
		  foreach ($list->result_array() as $r) {
			  ...
			  $data[] = array($no, $r['name'], $opt_grup[$r['id_admin_grup']], $check);
		  }
	  ...
	  function admin_detail($id=0)
		{
			...
			if($id) {
				...
				$data['labelz'] = 'Update User';
				$data['val'] 	= $q;
			} else {
				$data['labelz'] = 'Insert a new User';
				$data['val'] 	= array();
			}
			$this->load->view('template', $data);
		}
	  ...
	  function admin_add()
	  {
	    ...
	    $post = $param = $this->input->post();
			...
			$cek_old_pass = GetValue("userpass","admin",array("id"=> "where/".$param['id']));
			if(!$cek_old_pass || ($cek_old_pass != $post["userpass"])) $post['userpass'] = MD5("BNI-".$post["userpass"]."-LIFE");
			if(!$param['id']) {
				$post['id'] = GetValue("id","admin",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
			}
			//echo $cek_old_pass."/".$post["userpass"]."<br>";
			//print_mz($post);
			if(!$param['id']) {
