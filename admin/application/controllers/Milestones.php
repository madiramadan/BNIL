...
class Milestones extends CI_Controller {
	...
	function milestones_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_milestones WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
...
unset($post['id']);
		...
		if(!$param['id']) {
			$post['id'] = GetValue("id","milestones",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		
		$cek_id = GetValue("id","milestones",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
