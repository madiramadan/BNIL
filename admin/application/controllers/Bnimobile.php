...
class Bnimobile extends CI_Controller {
    ...
    $data['main'] = 'bnimobile';
    $data['filez'] = 'bnimobile';
    $data['functionz'] = 'tepat';
    $data['labelz'] = 'BNI Life Mobile';
	...
	function tepat_list($trash=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query_no_limit = "SELECT * FROM kg_bnimobile WHERE is_delete=0 AND ".$where." ORDER BY ".$ord;
		$query = "SELECT * FROM kg_bnimobile WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
	  ...
	  foreach ($list->result_array() as $r) {
		  ...
		  $check = "<a href='".site_url('bnimobile/tepat_detail/'.$r['id'])."'><i class='icon icon-pencil'></i></a>";
  ...
  function tepat_detail($id=0)
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'bnimobile_detail';
    $data['filez'] = 'bnimobile';
    $data['functionz'] = 'tepat';
    $data['labelz'] = 'BNI Life Mobile';
    $data['id'] = $id;
    if($id) {
    	$q = GetAll("bnimobile", array("id"=> "where/".$id))->result_array()[0];
    	$data['val'] = $q;
    } else $data['val'] = array();
    $this->load->view('template', $data);
	}
	...
	function tepat_aktif()
	{
		...
		if($idz) {
			if($this->input->post("akt")==2) {
				...
				$this->db->update("bnimobile", array("is_delete"=> 1));
			}
		}
	}
  ...
  function tepat_add()
  {
    ...
    $post['contents'] = "";
		...
		if(!$param['id']) {
			//$post['id'] = GetValue("id","bnimobile",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		...
		$cek_id = GetValue("id","bnimobile",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
    	$post['create_user_id'] = $admin_id;
			$post['create_date'] = date("Y-m-d H:i:s");
			$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			if($this->db->insert("bnimobile", $post)) $this->session->set_flashdata("message", "success-Add Success");
    	else $this->session->set_flashdata("message", "danger-Add Failed");
    } else {
    	$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			$this->db->where("id", $param['id']);
    	if($this->db->update("bnimobile", $post)) $this->session->set_flashdata("message", "success-Edit Success");
    	else $this->session->set_flashdata("message", "danger-Edit Failed");
    }
    redirect(site_url('bnimobile/tepat'));
  }
}
