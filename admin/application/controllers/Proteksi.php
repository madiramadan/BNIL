...
class Proteksi extends CI_Controller {
    ...
    $data['main'] = 'proteksi_tepat';
    $data['filez'] = 'proteksi';
    $data['functionz'] = 'tepat';
    $data['labelz'] = 'Proteksi Tepat';
	...
	function tepat_list($trash=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query_no_limit = "SELECT * FROM kg_proteksi_tepat WHERE is_delete=0 AND ".$where." ORDER BY ".$ord;
		$query = "SELECT * FROM kg_proteksi_tepat WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
	  ...
	  foreach ($list->result_array() as $r) {
		  ...
		  $check = "<a href='".site_url('proteksi/tepat_detail/'.$r['id'])."'><i class='icon icon-pencil'></i></a>";
  ...
  function tepat_detail($id=0)
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'proteksi_tepat_detail';
    $data['filez'] = 'proteksi';
    $data['functionz'] = 'tepat';
    $data['labelz'] = 'Proteksi Tepat';
    $data['id'] = $id;
    if($id) {
    	$q = GetAll("proteksi_tepat", array("id"=> "where/".$id))->result_array()[0];
    	$data['val'] = $q;
    } else $data['val'] = array();
    $this->load->view('template', $data);
	}
	...
	function tepat_aktif()
	{
		...
		if($idz) {
			if($this->input->post("akt")==2) {
				...
				$this->db->update("proteksi_tepat", array("is_delete"=> 1));
			}
		}
	}
  ...
  function tepat_add()
  {
    ...
    $post['contents'] = "";
		...
		if(!$param['id']) {
			//$post['id'] = GetValue("id","proteksi_tepat",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		...
		$cek_id = GetValue("id","proteksi_tepat",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
    	$post['create_user_id'] = $admin_id;
			$post['create_date'] = date("Y-m-d H:i:s");
			$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			if($this->db->insert("proteksi_tepat", $post)) $this->session->set_flashdata("message", "success-Add Success");
    	else $this->session->set_flashdata("message", "danger-Add Failed");
    } else {
    	$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			$this->db->where("id", $param['id']);
    	if($this->db->update("proteksi_tepat", $post)) $this->session->set_flashdata("message", "success-Edit Success");
    	else $this->session->set_flashdata("message", "danger-Edit Failed");
    }
    redirect(site_url('proteksi/tepat'));
  }
}
