...
class Tenaga_pemasar extends CI_Controller {
	...
	function tenaga_pemasar_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_tenaga_pemasar WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
...
unset($post['id']);
		...
		// upload file
		$config['upload_path']          = '../uploads/tenaga_pemasar';
		...
		$cek_id = GetValue("id","tenaga_pemasar",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
...
$post['modify_date'] = date("Y-m-d H:i:s");
			...
			$record = GetAll('tenaga_pemasar',array('id'=>'where/'.$cek_id))->row_array();
			if (!$this->upload->do_upload('userfile')){
