...
class Faq extends CI_Controller {
	...
	function faq_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_faq WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
		...
		foreach ($list->result_array() as $r) {
			...
			$data[] = array($no, $r['nama'], $r['urutan'], $check);
		}
...
unset($post['id']);
		...
		if(!$param['id']) {
			$post['id'] = GetValue("id","faq",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
			
		$cek_id = GetValue("id","faq",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
...
$post['modify_date'] = date("Y-m-d H:i:s");
	...
	function view_list($id_faq=0){
		...
		$query 	= "SELECT * FROM kg_faq_view WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." OFFSET ".$_POST['start']." ROWS FETCH NEXT ".$_POST['length']." ROWS ONLY";;
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
			...
			$data[] = array($no, $r['question'], $r['answer'], $r['create_date'], $r['modify_date'], $r['urutan'], $check);
		}
...
unset($post['id']);
		...
		if(!isset($post['is_delete'])) $post['is_delete']=0;
		if(!$param['id']) {
			$post['id'] = GetValue("id","faq_view",array("limit"=> "0/1", "id"=> "order/desc")) + 1;
		}
		
		$cek_id = GetValue("id","faq_view",array("id"=> "where/".$param['id']));
		if(!$cek_id) {
			$post['create_user_id'] = $admin_id;
			$post['create_date'] = date("Y-m-d H:i:s");
$post['modify_user_id'] = $admin_id;
