                	...
                	<form class="form-material" method="POST" action="<?php echo site_url($filez.'/'.$functionz.'_add');?>" enctype="multipart/form-data">
	                	...
	                	<div class="row">
							...
							<div class="col-md-3 box-content box-content-kanan">
								...
								<div class="form-group form-float">
									<div>
										...
										<?php if(isset($val['brosur']) && $val['brosur']){ ?>
											<a href="<?php echo base_url('../uploads/product/'.$val['brosur']); ?>" target="_blank"><?php echo $val['brosur']; ?></a> | <a href="#" onclick="delbrosur(<?php echo $val['id']?>)"><i class="fa fa-trash"></i>Hapus Brosur</a><br>
										<?php } ?>
								...
								<div class="form-group form-float">
									<div>
										<label class="form-label">Riplay Umum</label>
										<input type="file" name="riplay" class="form-control">
										<?php if(isset($val['title_image_mobile']) && $val['title_image_mobile']){ ?>
											<a href="<?php echo base_url('uploads/'.$functionz.'/'.$val['title_image_mobile']); ?>" target="_blank"><?php echo $val['title_image_mobile']; ?></a><br>
										<?php } ?>
										<span class="badge badge-danger" style="margin-top: 10px;">*File harus berformat PDF dengan Max Size 45MB<span>
									</div>
								</div>
...
<script>
	...
	function delbrosur(id){
	
		if (confirm("Anda Yakin Akan Menghapus Brosur?") == true) {
			$.post('<?php echo base_url()?>product/deletebrosur',{id:id},function(e){
				if(e=='ok'){
					location.reload();
				}else{
