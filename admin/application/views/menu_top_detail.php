              ...
              <div class="card-box">
                	<form class="form-material" method="POST" action="<?php echo site_url($filez.'/'.$functionz.'_add');?>" enctype="multipart/form-data">
                        ...
                        <div class="form-group form-float">
                            ...
                            <?php
                            if(isset($val['logo'])) {
                            	if(file_exists("../uploads/".$val['logo']) && $val['logo']) echo "<br><img src='".str_replace("admin","",base_url())."uploads/".$val['logo']."' style='max-height:150px;'>";
                            }
                            ?>
                          </div>
                        </div>
                        <div class="form-group form-float">
					            		<div>
					            			<label class="form-label">Logo/Image White</label>
                            <input type="file" name="logo_white" accept="image/*" class="form-control">
                            <?php
                            if(isset($val['logo_white'])) {
                            	if(file_exists("../uploads/".$val['logo_white']) && $val['logo_white']) echo "<br><img src='".str_replace("admin","",base_url())."uploads/".$val['logo_white']."' style='max-height:150px;'>";
                            }
                            ?>
                          </div>
                        </div>
