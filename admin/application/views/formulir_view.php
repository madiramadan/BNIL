    ...
    <div class="animated fadeInUpShort">
        <div class="card" style="background:white;">
        	<div class="col-md-12 p-b-20 text-red">
                <a class="text-white btn btn-success btn-sm" href="<?php echo site_url($filez.'/'.$functionz.'_form/'.$id_formulir.'/0');?>">Add New</a> &nbsp; <?php echo $labelz;?>
            </div>
...
$(document).ready(function() {
  //datatables
    table = $('#table-edit').DataTable({
        ...
        "ajax": {
            "url" : "<?php echo site_url($filez.'/'.$functionz.'_list/'.$id_formulir);?>",
            "type": "POST"
        },
    });
    $('#table-edit').on('click', '.akt', function () {
        ...
        if(confirm('Are your sure '+lbl+' ?')) {
            $.post("<?php echo site_url($filez.'/'.$functionz.'_aktif/'.$id_formulir); ?>", {
                idz : rel, 
                akt : zz
            }, function(response) {
                window.location = "<?php echo site_url($filez.'/'.$functionz.'/'.$id_formulir); ?>";
            });
        } else return false;
    });
