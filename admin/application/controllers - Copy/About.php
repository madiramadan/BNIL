...
class About extends CI_Controller {
  ...
  function add()
  {
    ...
    redirect(site_url('about/main/'.$post['id']));
  }
	...
	function blog_list($cat=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query = "SELECT * FROM kg_news WHERE 1 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function blog_add()
  {
    ...
    $post = $this->input->post();
		...
		//File
		$key="file";
		$file_up = $post['id']."-".$_FILES[$key]['name'];
		$myfile_up	= $_FILES[$key]['tmp_name'];
		$ukuranfile_up = $_FILES[$key]['size'];
