...
class formulir extends CI_Controller {
	...
	function formulir_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_formulir WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function formulir_add()
	{
		...
		$cek_id = GetValue("id","formulir",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
	...
	function view_list($id_formulir=0){
		ini_set('memory_limit', -1);
		$where  = '1 AND id_formulir = '.$id_formulir;
		...
		$query 	= "SELECT * FROM kg_formulir_view WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
	...
	function view_form_save(){
		...
		// upload file
		$config['upload_path']          = './assets/uploads/formulir';
		...
		$cek_id = GetValue("id","formulir_view",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
