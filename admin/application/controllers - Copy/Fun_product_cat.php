...
class fun_product_cat extends CI_Controller {
	...
	function fun_product_cat_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_fun_product_cat WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		...
		foreach ($list->result_array() as $r) {
			...
			$data[] = array($no, $strip.$r['title'], $check);
		}
	...
	function fun_product_cat_add()
	{
		...
		$cek_id = GetValue("id","fun_product_cat",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
