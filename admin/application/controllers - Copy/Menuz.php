...
class Menuz extends CI_Controller {
  ...
  function top_add()
  {
    ...
    $post = $this->input->post();
		...
		$cek_id = GetValue("id","menu_top",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
    	...
    	$post['modify_user_id'] = $admin_id;
			...
			$parents = GetValue("id_parents","menu_top",array("id"=> "where/".$post['id']));
  ...
  function bottom_add()
  {
    ...
    $post = $this->input->post();
		...
		$cek_id = GetValue("id","menu_bottom",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
    	$post['create_user_id'] = $admin_id;
			$post['create_date'] = date("Y-m-d H:i:s");
    	...
    	$post['modify_user_id'] = $admin_id;
			$post['modify_date'] = date("Y-m-d H:i:s");
			//Cek Id Parents Perubahan
			$parents = GetValue("id_parents","menu_bottom",array("id"=> "where/".$post['id']));
			if($parents!=$post['id_parents']) {
				$urut = GetValue("urutan","menu_bottom",array("id_parents"=> "where/".$post['id_parents'], "urutan"=> "order/desc", "limit"=> "0/1"));
				$post['urutan'] = $urut+1;
			}			
