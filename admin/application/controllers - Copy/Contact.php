...
class Contact extends CI_Controller {
	...
	function contact_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_contact WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function contact_add()
	{
		...
		$cek_id = GetValue("id","contact",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
	...
	function view_list($id_contact=0){
		ini_set('memory_limit', -1);
		$where  = '1 AND id_contact = '.$id_contact;
		...
		$query 	= "SELECT * FROM kg_contact_view WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
			...
			$data[] = array($no, $r['nama_eng'], $r['kup'], $r['address'], $r['city'], $check);
		}
	...
	function view_form_save(){
		...
		$cek_id = GetValue("id","contact_view",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
			$post['create_user_id'] = $admin_id;
			$post['create_date']    = date("Y-m-d H:i:s");
