...
class Merchandise_ban extends CI_Controller {
	...
	function merchandise_ban_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_merchandise_ban WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function merchandise_ban_add()
	{
		...
		$cek_id = GetValue("id","merchandise_ban",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
