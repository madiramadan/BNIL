...
class Career extends CI_Controller {
	...
	function career_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_career WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function career_add()
	{
		...
		$cek_id = GetValue("id","career",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
	...
	function category_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_career_category WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
	...
	function category_add()
	{
		...
		$cek_id = GetValue("id","career_category",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
			$post['create_user_id'] = $admin_id;
			$post['create_date']    = date("Y-m-d H:i:s");
	...
	function level_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_career_level WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
	...
	function level_add()
	{
		...
		$cek_id = GetValue("id","career_level",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
			$post['create_user_id'] = $admin_id;
			$post['create_date']    = date("Y-m-d H:i:s");
