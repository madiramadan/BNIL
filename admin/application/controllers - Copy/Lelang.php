...
class Lelang extends CI_Controller {
	...
	function lelang_list($trash=0)
	{
		...
		$where = 1;
		$kolom = array("","title","file","periode","create_date");
		...
		$query 	= "SELECT * FROM kg_lelang WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
			$no++;
			$file 		= !empty($r['file']) ? '<a href="'.base_url('assets/uploads/lelang/'.$r['file']).'" target="_blank" class="btn btn-info btn-xs">View</a>' : '';
	...
	function lelang_add()
	{
		...
		$cek_id = GetValue("id","lelang",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
