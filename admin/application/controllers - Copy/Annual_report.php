...
class Annual_report extends CI_Controller {
	...
	function annual_report_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_annual_report WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function annual_report_add()
	{
		...
		// upload file
		$config['upload_path']          = './assets/uploads/annual_report';
		...
		$cek_id = GetValue("id","annual_report",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
