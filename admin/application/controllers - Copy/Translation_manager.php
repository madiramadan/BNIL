...
class Translation_manager extends CI_Controller {
	...
	function translation_manager_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_translation_manager WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function translation_manager_add()
	{
		...
		$cek_id = GetValue("id","translation_manager",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
