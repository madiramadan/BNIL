...
class Email_recipient extends CI_Controller {
	...
	function email_recipient_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_email_recipient WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	...
	function email_recipient_add()
	{
		...
		$cek_id = GetValue("id","email_recipient",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
