...
class Management extends CI_Controller {
	...
	function management_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_management WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		...
		foreach ($list->result_array() as $r) {
			...
			$data[] 	= array($no, $r['name'], $r['position_eng'], $r['category_eng'], $check);
		}
  	...
  	function management_detail($id=0)
	{
		...
		$data['filez'] 		= 'management';
		$data['functionz'] 	= 'management';
		$data['category'] 	= GetAll('management_category')->result_array();
		$data['id'] 		= $id;
		if($id) {
	...
	function management_add()
	{
		...
		$cek_id = GetValue("id","management",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
	...
	function categoryman_list($trash=0)
	{
		...
		$query 	= "SELECT * FROM kg_management_category WHERE is_delete = 0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
		$list 	= $this->db->query($query);
		$data 	= array();
		$no 	= $_POST['start'];
		foreach ($list->result_array() as $r) {
	...
	function categoryman_add()
	{
		...
		$cek_id = GetValue("id","management_category",array("id"=> "where/".$post['id']));
		if(!$cek_id) {
			$post['create_user_id'] = $admin_id;
			$post['create_date']    = date("Y-m-d H:i:s");
